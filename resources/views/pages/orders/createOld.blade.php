...
function add_product(){
	...
	//$('#add_product_row').append('<div id="remove_'+fileId+'"> <hr/>'+data+remove_button+'</div>');

	$('#add_product_row').append(dynamic_row);
}
...
$('#searched_attribute_id').change(function(){
	...
	$.ajax({
		...
		success:function(data){
			...
			$('#base_price').parent('label').find('span').html(prices.price+' Base Price');
			$('#sell_price').val(prices.sell_price);
			$('#sell_price').parent('label').find('span').html(prices.sell_price+' Sell Price');
			$('#merchant_price').val(prices.merchant_price);
			$('#merchant_price').parent('label').find('span').html(prices.merchant_price+' Cust Price');
