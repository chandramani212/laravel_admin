...
*/				
				...
				'<td>'+
					'<a href="javascript:remove_row(\''+productRowId+'\');" class="btn btn-warning default btn-sm">'+
...
function getAttributePrice(elem){
	...
	$.ajax({
		...
		success:function(data){
			...
			if(prices.specific_price > 0 || prices.custom_price > 0 ){
				if(prices.default_customer_specific_price == 'CUSTOM_PRICE'){
					$(elem).parent('td').parent('tr').find('.merchant_price input').attr('checked',true);

					$(elem).parent('td').parent('tr').find('.merchant_price').find('#CUSTOM_PRICE').val(prices.specific_price);
					...
					$(elem).parent('td').parent('tr').find('.specific_price').find('#SPECIFIC_PRICE').val(prices.sell_price);
					$(elem).parent('td').parent('tr').find('.specific_price input.specific_price').val(prices.specific_price);
				}
			}
